            ...
            <div key={it.id} className="rounded-2xl overflow-hidden" style={{ background: "#FFFFFF", boxShadow: "0 1px 3px rgba(10,25,48,0.08)" }}>
              <div className="flex items-stretch">
                ...
                <div className="flex-1 p-2.5 space-y-1.5">
                  ...
                  <input value={it.name} onChange={(e) => updateBulk(it.id, "name", e.target.value)} placeholder="Product name" autoCapitalize="words" className="w-full bg-transparent border-b pb-1 text-[13px] font-sans outline-none" style={{ borderColor: TOKENS.line, color: TOKENS.inkDeep, background: "#FFFFFF" }} />
  ...
  if (screen === "addProduct") {
    ...
    return (
      <Shell>
        <button onClick={() => setScreen("list")} className="font-mono text-xs mb-6 flex items-center gap-1 rounded-full border px-3 py-1" style={{ color: TOKENS.ink, opacity: 0.68, background: "#FFFFFF", borderColor: TOKENS.line }}><ArrowLeft size={13} /> back</button>
        <h2 className="font-display font-semibold text-xl mb-6" style={{ color: TOKENS.inkDeep }}>Add Product</h2>
        <TextInput label="Product Name" value={newProduct.name} onChange={(e) => setNewProduct((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. Tata Salt 1kg" autoCapitalize="words" />
        <TextInput label="Product Code" value={newProduct.code} onChange={(e) => setNewProduct((p) => ({ ...p, code: e.target.value }))} placeholder="P-105" />
        <TextInput label="Category (Optional)" value={newProduct.category} onChange={(e) => setNewProduct((p) => ({ ...p, category: e.target.value }))} placeholder="Grocery" autoCapitalize="words" />
        <div className="mb-4">
